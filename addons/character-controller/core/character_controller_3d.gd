...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, input_swim_down := false, input_swim_up := false, charShadow:RigidBody3D=null) -> void:
	...
	if(!charShadow):
		...
		direction.y = 0
		velocity.y += gravity * _delta
		charShadow.apply_force(direction.normalized()*speed*charShadow.mass*1.5)
		if(jump_ability.is_actived()):
			var velY: float = sqrt(2.0 * charShadow.get_gravity().length() * velocity.y*0.3)
			charShadow.apply_central_impulse(charShadow.mass * velY * up_direction)
