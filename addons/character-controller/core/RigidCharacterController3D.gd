extends SmoothRigidCharacterBody3D
class_name RigidCharacterController3D
...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, input_swim_down := false, input_swim_up := false) -> void:
	...
	if not jump_ability.is_actived() and not is_fly_mode() and not is_submerged() and not is_floating():
		target_velocity.y -= gravity * _delta
		#target_velocity.y = 0
	...
	for ability in _abilities:
		target_velocity = ability.apply(target_velocity, speed, is_on_floor(), direction, _delta)
	
	_horizontal_velocity = Vector3(target_velocity.x, 0.0, target_velocity.z)
	if(target_velocity.y>0):
		var velocity: float = sqrt(2.0 * get_gravity().length() * target_velocity.y)
		apply_central_impulse(mass * velocity * up_direction)
	target_velocity = _horizontal_velocity
	move_and_slide()
