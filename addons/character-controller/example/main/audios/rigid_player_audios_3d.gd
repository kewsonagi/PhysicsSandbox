extends Node3D
class_name RigidPlayerAudios3D
...
@onready var character_body: RigidCharacterBody3D = get_node(NodePath(".."))
@onready var character_controller: RigidCharacterController3D = get_node(NodePath(".."))
...
func _get_audio_interact():
	if(character_body.get_colliding_bodies() and character_body.get_colliding_bodies().size()>0):
		#var k_col = character_body.get_last_slide_collision()
		#var collision = k_col.get_collider(0)
		var collision = character_body.get_colliding_bodies()[0]
		_get_audio_interact_of_object(collision)
	return
