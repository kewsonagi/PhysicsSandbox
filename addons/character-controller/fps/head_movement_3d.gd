...
@export var shadowBodyNode: Node3D
...
func _ready() -> void:
	...
	if(!cameraNode):
		...
		if(!cameraNode):
			cameraNode = get_node_or_null(NodePath("Head/FirstPersonCameraReference/Camera3D"))
	actual_rotation.y = cameraNode.rotation.y
...
func rotate_camera(mouse_axis : Vector2) -> void:
	...
	#cameraNode.rotation.y = actual_rotation.y
	bodyNode.rotation.y =  actual_rotation.y
	if(shadowBodyNode):
		shadowBodyNode.rotation.y =  actual_rotation.y
