...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, input_swim_down := false, input_swim_up := false, charShadow:RigidBody3D=null):
	if is_fly_mode() or is_floating():
		_direction_base_node = head
	else:
		_direction_base_node = self
	super.move(_delta, input_axis, input_jump, input_crouch, input_sprint, input_swim_down, input_swim_up, charShadow)
